...
def U3(AA, BB):
    """ http://www.lix.polytechnique.fr/Labo/Francois.Morain/Drafts/ccr.pdf """
    u3 = x^4 +2*AA*x^2+4*BB*x - AA^2/3
    return u3

def U5(AA, BB):
    """ http://www.lix.polytechnique.fr/Labo/Francois.Morain/Drafts/ccr.pdf, 
    the root of this polynomial is the p1 coefficient of the kernel polynomial """
    u5 = x^6 + 20*AA*x^4 + 160*BB*x^3 - 80*AA^2*x^2 - 128*AA*BB*x - 80*BB^2
    return u5
...
def weilpairingmanual(m, x1, y1, x2, y2):
...
print weilpairing(6, 121, 387, 36, 60)
print weilpairing(6, 36, 60, 121, 387)
